...
export default function Header() {
    ...
    const handleNotificationClick = () => {
        // Your logic to handle notification click goes here
        // For example, you can fetch notifications from an API
        // and update the state
        // For demonstration, let's assume notifications are fetched from an API
        const fetchedNotifications = ["Notification 1"]; // fetched notifications from API
        setNotifications(fetchedNotifications); // Update notifications state
                ...
                <div className="top-full right-0 absolute bg-white shadow-md mt-2 rounded-md w-44">
                    {notifications.length === 0 ? (
                        <p className="px-4 py-2 text-gray-800">
                            There's no notification received.
                        </p>
                    ) : (
                        notifications.map((notification, index) => (
                            <button
                                key={index}
                                className="flex items-center hover:bg-gray-200 px-4 py-2 w-full text-gray-800 text-left"
                            >
                                {notification}
                            </button>
                        ))
